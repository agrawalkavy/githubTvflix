...
const getGenres = function (gnereList) {
    const newGenreList = [];

    for (const { name } of gnereList) newGenreList.push(name);
...
const getCasts = function (castList) {
    ...
    for (let i = 0, leng = castList.length; i < leng && i < 10; i++) {
...
fetchDataFromServer(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${api_key}&append_to_response=casts,videos,images,releases`, function (movie) {
    ...
    fetchDataFromServer(`https://api.themoviedb.org/3/movie/${movieId}/recommendations?api_key=4ae266f8a6e77742156847ee79a4b573&page=1`, addSuggestedMovies);
